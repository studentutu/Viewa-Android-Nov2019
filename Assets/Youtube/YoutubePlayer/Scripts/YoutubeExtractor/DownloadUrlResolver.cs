...
namespace YoutubeExtractor
{
    ...
    public static class DownloadUrlResolver
    {
        ...
        static IEnumerable<VideoInfo> MyGetVideoInfos(JObject json)
        {
            ...
            Debug.Log(json2);/* Debug playabilityStatus (When I record this tutorial I was not able to stream videos with 2160p
                                 resolution, if you need to stream in 4K here is where you need to make changes until you get it,
                                 if I found a way to do that i the future I update the code)*/

            var list = new List<VideoInfo>();

            foreach (var item in json2["streamingData"]["adaptiveFormats"])
            {
                var videoInfo = VideoInfo.Defaults.SingleOrDefault(x => x.FormatCode == (int)item["itag"]);
        ...
        public static IEnumerable<VideoInfo> GetDownloadUrls(string videoUrl, bool decryptSignature = true)
        {
            ...
            try
            {
                Debug.Log("111111111111111111: " + videoUrl);
