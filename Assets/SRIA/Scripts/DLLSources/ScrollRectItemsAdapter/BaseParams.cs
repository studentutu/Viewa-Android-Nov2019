...
namespace frame8.Logic.Misc.Visual.UI.ScrollRectItemsAdapter
{
   ...
   //     }
        ...
        public BaseParams(MyScrollRect scrollRect)
            :this(scrollRect, scrollRect.transform as RectTransform, scrollRect.content)
        {}

        public BaseParams(MyScrollRect scrollRect, RectTransform viewport, RectTransform content)
        {
		...
		public virtual void InitIfNeeded(ISRIA sria)
        {
			...
			if (!scrollRect)
				scrollRect = sria.AsMonoBehaviour.GetComponent<MyScrollRect>();
			if (!scrollRect)
