...
#pragma warning restore
	...
	private void DrawMiscSection ()
	{
...
#elif UNITY_IOS
					AppendResult(string.Format("PublicKeyURL = {0}", (_credentials.iOSCredentials.PublicKeyURL != null) ? _credentials.iOSCredentials.PublicKeyURL : null));
					AppendResult(string.Format("Signature = {0}", (_credentials.iOSCredentials.Signature != null) ? System.BitConverter.ToString(_credentials.iOSCredentials.Signature) : null));
					AppendResult(string.Format("Salt = {0}", (_credentials.iOSCredentials.Salt != null) ? System.BitConverter.ToString(_credentials.iOSCredentials.Salt) : null));
					AppendResult(string.Format("Timestamp = {0}", _credentials.iOSCredentials.Timestamp));
#endif
				}

                StartCoroutine(VerifySignature(_credentials));
			});
		}
        ...
        IEnumerator VerifySignature(ExternalAuthenticationCredentials credentials)
        {
            ...
            UnityWebRequest www = UnityWebRequest.Post("https://us-central1-gamecenterauth.cloudfunctions.net/authenticate", form);
