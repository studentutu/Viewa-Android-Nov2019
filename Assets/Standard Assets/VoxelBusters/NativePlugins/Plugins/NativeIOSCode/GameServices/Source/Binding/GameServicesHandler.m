...
const NSString *kGameServicesExternalAuthenticationCredentials                      = @"external-authentication-credentials";
const NSString *kGameServicesExternalAuthenticationCredentialsPublicKeyUrl          = @"public-key-url";
const NSString *kGameServicesExternalAuthenticationCredentialsSignature             = @"signature";
const NSString *kGameServicesExternalAuthenticationCredentialsSalt                  = @"salt";
const NSString *kGameServicesExternalAuthenticationCredentialsTimestamp             = @"timestamp";
...
#define kLoadExternalAuthCredentialsFinished    "LoadExternalAuthenticationCredentialsFinished"
...
- (void)loadExternalAuthenticationCredentials
{
    [[GKLocalPlayer localPlayer] generateIdentityVerificationSignatureWithCompletionHandler:^(NSURL *publicKeyUrl, NSData *signature, NSData *salt, uint64_t timestamp, NSError *error)
     {
         ...
         if(publicKeyUrl != nil)
         {
             ...
             [credentials setObject:[publicKeyUrl absoluteString] forKey:kGameServicesExternalAuthenticationCredentialsPublicKeyUrl];
             [credentials setObject:signatureString forKey:kGameServicesExternalAuthenticationCredentialsSignature];
             [credentials setObject:saltString forKey:kGameServicesExternalAuthenticationCredentialsSalt];
             [credentials setObject: [NSString stringWithFormat:@"%llu", timestamp] forKey:kGameServicesExternalAuthenticationCredentialsTimestamp];
         }

         [dataDict setObject:credentials forKey:kGameServicesExternalAuthenticationCredentials];

         if (error != NULL)
             [dataDict setObject:[error description] forKey:kGameServicesError];

         NotifyEventListener(kLoadExternalAuthCredentialsFinished, ToJsonCString(dataDict));
     }
     ];
}
